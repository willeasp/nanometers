...
/// The data types a Module consumes — `Rect`, `Measurements`, `FrameContext` (and `StereoFrame`) —
/// now live in `nano-dsp` (ADR 0008): they're platform-free, shared with the TUI and iOS. Re-exported
/// here so the concrete Modules keep referring to `super::{FrameContext, Rect, …}` unchanged. The
/// `Module` trait stays in this crate because its signatures name `wgpu` types.
