...
unsafe fn create_view_class() -> &'static Class {
    ...
    // Selector target for the AppKit display link (see `WindowState::setup_frame_driver`): fired
    // once per vsync, drives one frame. Replaces the old 66.7 Hz CFRunLoopTimer cadence.
    class.add_method(
