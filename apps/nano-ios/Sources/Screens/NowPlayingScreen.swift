...
struct NowPlayingScreen: View {
    ...
    var body: some View {
        ...
        .task(id: engine.current?.binsTaskID) {   // re-runs on track switch + once a cloud/Drive track is analyzed
            ...
            Button { engine.toggle() } label: {
                ZStack {
                    ...
                    if engine.isPreparing {
                        ProgressView().controlSize(.regular).tint(Theme.bg)             // Drive track downloading/preparing
                    } else {
    ...
    /// §06 bottom rail: Go-to-Source folder · Settings · AirPlay · queue. AirPlay kept per the user
    /// override of the handoff's 3-icon rail. (Go-to-Source wiring from the library/Drive lane.)
